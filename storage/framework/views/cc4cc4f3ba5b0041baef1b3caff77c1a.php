...
<link rel="stylesheet" href="<?php echo e(asset('assets/css/audio.css')); ?>">
...
<div class="profiles-container overflowww" style="overflow-y: auto !important;max-height: 53vh !important;">
    ...
    <div class="profile-row">
        <?php $__currentLoopData = $media; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php if(canViewProfile(auth()->user(), $item->user)): ?>
        <div class="profile-card" id="audio-card-<?php echo e($item->id); ?>" style="position: relative;">
            <div style="position: absolute; top: 2px; right: 14px;">
                <a href="<?php echo e(url('user/profile/' . $item->user->id)); ?>">
                    <img src="<?php echo e(asset($item->user->profile_picture)); ?>" alt="<?php echo e($item->user->name); ?>" style="width: 40px; height: 40px; border-radius: 50%; border: 2px solid white;">
                </a>
            </div>

            <div id="pin-badge-<?php echo e($item->id); ?>" style="position: absolute; margin-left:143px; top:5px;  z-index: 2;">
                <?php if($item->pins->where('user_id', auth()->id())->where('is_pinned', 1)->count()): ?>
                        <img src="<?php echo e(asset('assets/img/pinned.svg')); ?>" alt="Pinned" style="width: 20px; height: 20px;" />
                <?php endif; ?>
            </div>

            <img src="<?php echo e(asset($item->image)); ?>" class="profile-image" data-audio="<?php echo e(asset($item->audio)); ?>" onclick="toggleAudio(this)">
            ...
            <div class="card-icons">
                ...
                <img src="assets/img/Share.svg" alt="Share" onclick="shareContent(<?php echo e($item->id); ?>)">

                <?php
                    $isPinned = $item->pins->where('user_id', auth()->id())->where('is_pinned', 1)->count() > 0;
                ?>

                <img src="<?php echo e(asset($isPinned ? 'assets/img/pinned.svg' : 'assets/img/pin.svg')); ?>" alt="pin"
                    onclick="pinAudio(<?php echo e($item->id); ?>, this)"
                    style="width:20px; height: 20px; filter: brightness(0) invert(1); " />

                <?php if(auth()->id() == $item->user_id): ?>  <!-- Check if logged-in user is the owner -->
                <div >
                    <img src="<?php echo e(asset('assets/img/delete.svg')); ?>" alt="Delete" style="width: 25px; height: 25px;" onclick="openDeleteModal(<?php echo e($item->id); ?>)">
                </div>
                <?php endif; ?>

            </div>

            <div id="likeCount-<?php echo e($item->id); ?>" style="font-size: 14px; margin-top: 4px; color:white; margin-left:-109px;">0</div>
            <div id="commentCount-<?php echo e($item->id); ?>" style="font-size: 14px; margin-top:-15px; color:white; margin-left:-37px;" >0</div>
...
<script>
    document.querySelector('#uploadModal form').addEventListener('submit', function(e) {
        ...
        fetch("<?php echo e(route('upload-audio')); ?>", { 
                method: 'POST'
                , headers: {
...
<script>
    ...
    function loadAudioLikes(audioId) {
        ...
        const allAudioIds = <?php echo json_encode($media->pluck('id'), 15, 512) ?>;
...
<script src="<?php echo e(asset('assets/js/audio.js')); ?>"></script>
<?php echo $__env->make('frontend.layout.footer', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>

<?php /**PATH E:\laragon\www\animation_social\animation_social_project\resources\views/frontend/signal_audio.blade.php ENDPATH**/ ?>